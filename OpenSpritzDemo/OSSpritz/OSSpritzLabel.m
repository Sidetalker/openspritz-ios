...
- (void)setUp
{
    ...
    for (int i = 0; i < 18; i++)
    {
        ...
        if (i==pivotChar) label.textColor = [UIColor greenColor]; // CUSTOM
        else label.textColor = [UIColor blueColor]; // CUSTOM
        label.alpha = 0;
        label.text = @"m";
    ...
    // Finish by drawing the pivot guide lines
    [self drawGuideLines];
}
...
//    NSArray *paths = @[upperGuidePath, downGuidePath]; // CUSTOMIZE
    
    for (UIBezierPath *path in paths)
    {
        CAShapeLayer *line = [CAShapeLayer layer];
        line.path=path.CGPath;
        line.lineWidth = 100.0f; // CUSTOMIZE
        line.fillColor = [UIColor greenColor].CGColor; // CUSTOMIZE
        line.opacity = 1.0;
